...
use google_youtube3::Result;
use google_youtube3::{hyper_rustls, hyper_util, yup_oauth2, YouTube};

use crate::api_handler::hyper_util::client::legacy::connect::HttpConnector;
use google_youtube3::yup_oauth2::authenticator::Authenticator;
use google_youtube3::yup_oauth2::ServiceAccountAuthenticator;
...
async fn get_auth() -> Authenticator<HttpsConnector<HttpConnector>> {
    let key = yup_oauth2::read_service_account_key("res/keys/client_secret.json")
        .await
        .unwrap();
...
pub async fn google_api(target: String) -> Result<Video> {
    if target.len() != 11 {
        //Going to be honest, not sure why this works but just returning the Err doesn't.
        let result = Err(Box::<dyn Error>::from(
            "Invalid API target, must be an 11 character video id.",
        ));
    ...
    match request.doit().await {
        Ok(res) => Ok(res.1.items.unwrap().pop().unwrap()),
